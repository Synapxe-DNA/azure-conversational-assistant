...
export class ChatFollowupService {
  ...
  async load(profileId: string): Promise<BehaviorSubject<FollowUp[]>> {
    ...
    const followUps = await firstValueFrom<FollowUp[] | undefined>(this.indexedStore.getAllByIndex("follow_up", "profile_id", IDBKeyRange.only(profileId)));
  ...
  async staticLoad(profileId: string): Promise<FollowUp[]> {
    return await firstValueFrom<FollowUp[]>(this.indexedStore.getAllByIndex("follow_up", "profile_id", IDBKeyRange.only(profileId)));
  }

  insert(followUp: FollowUp): Promise<void> {
    if (followUp.profile_id !== this.$currentProfileId.value) {
      console.warn("[FollowupService] Attempting to insert question that does not match current profile ID!");
    }
