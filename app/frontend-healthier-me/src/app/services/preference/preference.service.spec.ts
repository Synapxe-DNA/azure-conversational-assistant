...
describe("PreferenceService", () => {
  ...
  it("should initialise localStorage", () => {
    expect(
      JSON.parse(localStorage.getItem(PreferenceKey.ChatMode) as string),
    ).toBe(service.$chatMode.value);
    expect(
      JSON.parse(
        localStorage.getItem(PreferenceKey.VoiceDetectInterrupt) as string,
      ),
    ).toBe(service.$voiceDetectInterrupt.value);
    expect(
      JSON.parse(
        localStorage.getItem(PreferenceKey.VoiceDetectStart) as string,
      ),
    ).toBe(service.$voiceDetectStart.value);
    expect(
      JSON.parse(localStorage.getItem(PreferenceKey.VoiceDetectEnd) as string),
    ).toBe(service.$voiceDetectEnd.value);
  });
