...
def main():
    ...
    if selected_file != "all":
        ...
        full_path = f"backend/test/" # Run all tests and functions
        print(f"Running all tests on {selected_type} backend")

    pytest_args = ["-v", "-s", "--local", full_path, "-n", "auto"] if selected_type == "local" else ["-v", "-s", full_path, "-n", "auto"]
...
def get_test_type_option():
    ...
    terminal_menu = TerminalMenu(options, title="Select the backend you want to test", menu_highlight_style=("bg_blue","bold"))
...
def get_test_file_option():
    ...
    terminal_menu = TerminalMenu(options, title="Select the test you want to run", menu_highlight_style=("bg_blue","bold"))
    menu_entry_index = terminal_menu.show()
    return options[menu_entry_index]
...
def get_test_function(test_functions:list[str]):
    ...
    terminal_menu = TerminalMenu(options, title="Select the test function you want to run", menu_highlight_style=("bg_blue","bold"))
    menu_entry_index = terminal_menu.show()
    return options[menu_entry_index]
