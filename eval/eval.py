...
def create_metrics(model, use_mode, threshold=0.7):
    ARmetric = AnswerRelevancyMetric(
        threshold=threshold,
        model=model,
        include_reason=True,
        async_mode=use_mode
    )
    Fmetric = FaithfulnessMetric(
        threshold=threshold,
        model=model,
        include_reason=True,
        async_mode=use_mode
    )
    CRmetric = ContextualRelevancyMetric(
        threshold=threshold,
        model=model,
        include_reason=True,
        async_mode=use_mode
...
if __name__ == "__main__":
    ...
    df['retrieval_context'] = df['source_chunks'].apply(lambda x: x.split("\n+++\n")) #convert chunks into list of chunks
        ...
        if use_mode == True:
            ...
            for attempt in range(retries):
                try:
                    ...
                    results_df.to_excel(f"{output_dir}/deepeval_results_{use_model}_{now.strftime('%Y-%m-%d')}_{now.strftime('%H-%M')}.xlsx", index=False)
